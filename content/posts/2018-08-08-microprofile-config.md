...
The system will first see if there is a System property with the key `myservice.hostname`, if not it will try Environment variables, then all `microprofile-config.property` files on the classpath. If it could not find it anywhere, it will fallback to the `defaultValue` in the annotation.
...
I can also use any of the other config sources to do this, for example, including a `microprofile-config.properties` in my example war file (like [this example](https://github.com/phillip-kruger/microprofile-extensions/blob/master/example/src/main/webapp/META-INF/microprofile-config.properties)),  or use my other custom config source and change this in memory.
